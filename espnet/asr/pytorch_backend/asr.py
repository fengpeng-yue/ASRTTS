...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        x = _recursive_to(batch, self.model_device)
        #x = x + (self.iteration,)
        ...
        logging.info('iteration={},grad norm={}'.format(self.iteration,grad_norm))
...
class CustomConverter(object):
    ...
    def __init__(self, subsampling_factor=1, dtype=torch.float32,args=None):
...
def train(args):
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        logging.info("not using pretrained model")
    ...
    #logging.info(model.subsample[0])
    # set torch device
    ...
    if args.parallel_mode == "ddp":
        ...
        model = DataParallel(model,device_ids=[i for i in range (args.ngpu)])



    logging.info(args.freeze_mods)
    if args.freeze_mods:
    ...
    if args.opt == "adadelta":
        ...
        from espnet.nets.pytorch_backend.las.optimizer import get_std_opt
        optimizer = get_std_opt(
            model_params, args.lr, args.final_lr, args.warmup_steps, args.stop_steps, \
                args.decay_rate, args.decay_steps, args.lr_factor
        )
    ...
    if args.num_encs == 1:
        ...
        converter = CustomConverter(1, dtype=dtype,args=args)
    else:
    ...
    if args.parallel_mode == "ddp":
        ...
        dataset=train_dataset,batch_size=1,num_workers=args.n_iter_processes,collate_fn=lambda x: x[0],sampler=train_sampler)
    else:
        train_iter = ChainerDataLoader(
    ...
    if args.parallel_mode == "ddp":
        if args.local_rank == 0:
            log_manager(trainer,args,valid_json,model,converter,load_cv,device,mtl_mode)
    else:
        log_manager(trainer,args,valid_json,model,converter,load_cv,device,mtl_mode)
    if use_sortagrad:
        trainer.extend(
...
def log_manager(trainer,args,valid_json,model,converter,load_cv,device,mtl_mode):
