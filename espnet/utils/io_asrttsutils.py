...
class LoadInputsAndTargetsASRTTS(object):
    ...
    def __init__(self, mode='asr',
        ...
        if utt_spk is not None:
            ...
            #logging.info(self.spk2label_dict)
    

    def __call__(self, batch,data_aug=True,speaker_mode=False):
        ...
        uttid_list = []  # List[str]
        # logging.info("speaker model:")
        # logging.info(speaker_mode)
        if speaker_mode:
            speaker_list=[]
        else:
            speaker_list=None
        for uttid, info in batch:
            #logging.info(uttid)
            if speaker_mode:
                speaker_list.append(info['utt2spk'])
            uttid_list.append(uttid)

            if self.load_input:
                # Note(kamo): This for-loop is for multiple inputs
                for idx, inp in enumerate(info['input']):
                    #logging.info(inp['feat'])
        ...
        # logging.info(speaker_list)
        if self.mode == 'asr':
            return_batch, uttid_list,speaker_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, uttid_list,speaker_list)
        ...
        if self.preprocessing is not None and data_aug:
        ...
        if speaker_list is not None:
            ...
            for speaker in speaker_list:
                speaker_id.append(self.spk2label_dict[speaker])
        # logging.info(uttid_list)
        # logging.info(speaker_list)
        # logging.info(speaker_id)
        if speaker_mode:
            return tuple(return_batch.values()) + (speaker_id,)
        else:
            return tuple(return_batch.values())

    def _create_batch_asr(self, x_feats_dict, y_feats_dict, uttid_list,speaker_list):
        ...
        if self.load_output:
            ...
            for j in range(len(xs)):
                input_nonzero_id = list(filter(lambda i: len(xs[j][i]) > 0, range(len(xs[j]))))
                # remove zero-length samples
                xs = [[x[i] for i in input_nonzero_id] for x in xs]
                uttid_list = [ uttid_list[i] for i in input_nonzero_id]
                ys = [ [y[i] for i in input_nonzero_id ] for y in ys ]
        ...
        if speaker_list is not None:
            speaker_list = [speaker_list[i] for i in nonzero_sorted_idx]
        ...
        return return_batch, uttid_list,speaker_list
